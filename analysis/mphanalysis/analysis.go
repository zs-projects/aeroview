...
func main() {
	...
	fmt.Fprintf(w, "Experiment ( 64 bits )\tCHD\tRecsplit\tOriginal Map\t\n")
	...
	for _, size := range sizes {
		...
		fmt.Fprintf(w, "Number of elements: %v\t%v\t%v\t%v\t\n", len(dataset), memUsageCHD.HeapAlloc, memUsageRECSPLIT.HeapAlloc, memUsageMap.HeapAlloc)
		fmt.Fprintf(w, "Number of elements: %v\t%v\t%v\t%v\t\n", len(dataset), chd.SizeInBytes(), rcsplit.SizeInBytes(), 0)
	}
	w.Flush()
